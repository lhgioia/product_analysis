...
There is a statistically significant correlation between price and ingredient count. I guess that makes sense. Let's visualize the correlation using the [plotly R package](https://plotly.com/r/) to give a nice hover effect.

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=7}
library(plotly)

p <- ggplot(ingCountsPrice, aes(count, priceperoz)) +
  geom_point(aes(text = product)) +
  geom_smooth() +
...
kable(rareIngredientDf, align = 'c')
```

That's 413/713 ingredients that are only in 1 product. We do, however, have some ingredients that are used in most of the products.
...
ggplot(ingCountTop, aes(ingredient, count)) +
  geom_bar(stat="identity") +
